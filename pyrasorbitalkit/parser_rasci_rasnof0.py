__author__ = 'Abel Carreras modificacions by Aaron Rodriguez'
...
def parser_rasci(output):
    ...
    for m in re.finditer('RAS-CI total energy for state', output):
        ...
        tot_energy = float(section_state[enum: enum + 50].split()[5])
            ...
            #- - - Exc energy gives problems when is not printed in output
            #- - - The same happens for the multiplicity.
            #- - - The error arise when RASMAN estimates negative excitation
            #- - - energies.
        ...
        #print('- - - AARON WAS HERE - - -' )
        # RAS-NOF energy
        tot_nof_energy = None
        enum = section_state.find('RAS-NOF  total energy for state')
        if enum > -1:
            tot_nof_energy = float(section_state[enum: enum + 56].split()[6])
        ...
        if tot_nof_energy is not None:
            state_dict.update({'total_nof_energy': tot_nof_energy})
